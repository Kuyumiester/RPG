...
pub fn mainMenu() ?void {
    ...
    if (file_names.items.len == 0) { // do we actually want this? is there a chance they'll want to quit? probably not.
        ...
        switch (selection.which_array) {
            ...
            1 => { // load an existing character
                ...
                //        open the file
                //
                // rebuild the file name with ".rpg" at the end so we can open the file (we only saved the character's name of each file when looping through the files)
    ...
    _ = file.readAll(std.mem.asBytes(&world.inventory.len)) catch unreachable; // change the size of your slice to change how many bytes to read
    _ = file.readAll(std.mem.asBytes(&world.inventory.quantities)) catch unreachable;
    _ = file.readAll(std.mem.asBytes(&world.inventory.ids)) catch unreachable;
    world.setShopData();

    //const len: u64 = @divExact(length, @divFloor(@typeInfo(T).int.bits - 1, 8) + 1);
...
fn newCharacter(dir: fs.Dir) void {
    ...
    pc.name = pc.name_buffer[0..new_character_name.len];

    //
    //      create the save file with starting data
    //
    const new_game_data: [asBytes(&world.inventory.len).len + asBytes(&world.inventory.quantities).len + asBytes(&world.inventory.ids).len]u8 = comptime init: {
        //
        //      construct new game data
        //

        const slice: []const struct {
            id: objects.Id,
            qty: devtools.Int(99),
        } = &.{
            .{ .id = objects.index("starscar shield"), .qty = 1 },
            .{ .id = objects.index("zeniba's solid gold monogram seal"), .qty = 1 },
            .{ .id = objects.index("fireball"), .qty = 1 },
            .{ .id = objects.index("firegorger"), .qty = 1 },
            .{ .id = objects.index("ice missile"), .qty = 1 },
            .{ .id = objects.index("arc spell"), .qty = 1 },
            .{ .id = objects.index("thunder spell"), .qty = 1 },
            .{ .id = objects.index("potion of magic energy"), .qty = 12 },
        };

        //  turn the user-friendly data structure into a data-friendly structure that's straightforward to use
        //
        const len: devtools.Int(world.inventory_capacity) = @as(devtools.Int(world.inventory_capacity), @intCast(slice.len));
        var ids: [world.inventory_capacity]objects.Id = undefined;
        for (ids[0..slice.len], slice) |*to, from| {
            to.* = from.id;
        }
        var qtys: [world.inventory_capacity]devtools.Int(99) = undefined;
        for (qtys[0..slice.len], slice) |*to, from| {
            to.* = from.qty;
        }

        var array: [asBytes(&len).len + asBytes(&world.inventory.quantities).len + asBytes(&world.inventory.ids).len]u8 = undefined;
        var cursor: usize = 0;

        for (&.{ asBytes(&len), asBytes(&qtys), asBytes(&ids) }) |bytes| {
            for (array[cursor .. cursor + bytes.len], bytes) |*to, from| {
                to.* = from;
            }
            cursor += bytes.len;
        }
    ...
    file.writeAll(&new_game_data) catch unreachable;

    file.seekTo(0) catch unreachable;

    // optimization, if i wanted, i could just load the starting data straight from memory, instead of putting it into storage just to read it from storage into memory
    // that'd probably be best, tbh
}

// for getting input for characters, we should copy the functions in input.zig and modify them for longer inputs

pub fn saveGameFile() void {
    ...
    file.seekTo(0) catch unreachable;
    file.writeAll(std.mem.asBytes(&world.inventory.len)) catch unreachable;
    file.writeAll(std.mem.asBytes(&world.inventory.quantities)) catch unreachable;
    file.writeAll(std.mem.asBytes(&world.inventory.ids)) catch unreachable;
}
